...
for path in [project_root, helper_dir, jmmd_helper_dir]:
...
from JMMD.helper.utils_GAN import WeightScheduler, CNNGenerator, post_val

# Import DANN specific components
from utils_CNN import DomainDiscForCNN, train_step_cnn_residual_dann, val_step_cnn_residual_dann

def main():
    parser = argparse.ArgumentParser(description="Unified DANN CNN training runner.")
    ...
    if args.save_dir:
        ...
        path_temp = os.path.join(current_dir, 'results_dann')
    ...
    train_metrics = {
        ...
        'train_est_loss': [],
        'train_disc_loss': [],
        'train_domain_loss': [], # adversarial loss
    ...
    for epoch in range(args.n_epochs):
        ...
        train_step_output = train_step_cnn_residual_dann(
            ...
            lower_range=args.lower_range, save_features=save_features,
        ...
        print(f"  Loss: {train_epoc_loss:.6f} | Est Loss (Src): {train_epoc_loss_est:.6f} | Adversarial Loss: {train_epoc_loss_domain:.6f}")
        ...
        if (epoch == epoch_min) or (epoch + 1 > epoch_min and (epoch - epoch_min) % epoch_step == 0) and epoch != args.n_epochs - 1:
            H_sample, epoc_val_return = val_step_cnn_residual_dann(
                model, domain_disc, loader_H_eval, loss_fn, args.lower_range,
                weights=weights, linear_interp=linear_interp
            )
        ...
        post_val(epoc_val_return, epoch, args.n_epochs, val_metrics, domain_weight=weights['adv_weight'])
    ...
    print(f"Unified DANN run completed successfully! Results stored in: {model_path}/{sub_folder}")
