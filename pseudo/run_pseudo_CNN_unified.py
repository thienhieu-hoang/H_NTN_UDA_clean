...
DEFAULT_SOURCE_SNR = 10         # Source SNR is typically fixed at 10dB in pseudo runs
DEFAULT_PSEUDO_FOLDER = 'pseudoChannelD30_A300'
# =====================================================================
...
jmmd_helper_dir = os.path.join(project_root, 'JMMD', 'helper')

for path in [project_root, helper_dir, domain_helper_dir, jmmd_helper_dir]:
...
from utils_pseudo import save_checkpoint_pseudo as save_checkpoint
from utils_pseudo import train_step_cnn_residual_pseudo, val_step_cnn_residual_pseudo
from utils_GAN import CNNGenerator, WeightScheduler, post_val

def main():
    parser = argparse.ArgumentParser(description="Unified Pseudo-labeling CNN Runner.")
    ...
    parser.add_argument('--pseudo-folder', type=str, default=DEFAULT_PSEUDO_FOLDER, help="Folder containing pseudo channel matrices")
    ...
    pseudo_data_file_path = os.path.abspath(os.path.join(project_root, 'generatedChan', 'MATLAB', args.pseudo_folder, f'SNR_{SNR}dB_{args.fda_win_h}x{args.fda_win_w}', 'matlabNTN.mat'))

    print(f"Loading Source dataset (fixed at {args.source_snr}dB): {source_data_file_path}")
    print(f"Loading Target dataset: {target_data_file_path}")
    print(f"Loading Pseudo dataset: {pseudo_data_file_path}")
    ...
    class_dict_pseudo = {
        'GAN_linear': DataLoaders(pseudo_file, indices_train_pseudo, indices_val_pseudo, 'li', args.batch_size),
        'GAN_practical': DataLoaders(pseudo_file, indices_train_pseudo, indices_val_pseudo, 'prac', args.batch_size),
        'GAN_ls': DataLoaders(pseudo_file, indices_train_pseudo, indices_val_pseudo, 'ls', args.batch_size)
    }
    ...
    for epoch in range(args.n_epochs):
        ...
        train_step_output = train_step_cnn_residual_pseudo(
        ...
        if (epoch == epoch_min) or (epoch + 1 > epoch_min and (epoch - epoch_min) % epoch_step == 0) and epoch != args.n_epochs - 1:
            H_sample, epoc_val_return = val_step_cnn_residual_pseudo(
    ...
    print(f"Unified Pseudo runner completed successfully! Results stored in: {model_path}/{sub_folder}")
