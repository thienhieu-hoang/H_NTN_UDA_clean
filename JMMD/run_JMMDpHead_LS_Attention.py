"""
====================================================================================================
JMMD with Multi-Layer Projection Heads for HA02 Attention Model (OpenNTN & MATLAB)
====================================================================================================

Overview:
---------
This script trains and evaluates the HA02 Transformer-Convolutional Attention Model with 
Non-Linear Projection Head Joint Maximum Mean Discrepancy (JMMD) Unsupervised Domain Adaptation 
(UDA) for 5G Non-Terrestrial Network (NTN) channel estimation.

Projection Head JMMD Architecture:
----------------------------------
Instead of calculating kernel discrepancies directly on raw intermediate representations, each 
extracted layer is routed through an adaptive, dedicated Non-Linear Projection Head network:
    
    Z_layer [B, d_in] ---> [Dense(hidden_dim) -> LayerNorm -> GeLU -> Dense(proj_dim)] ---> P_layer [B, proj_dim]
                                                                                                 │
                                                                   JMMD Multi-Kernel RKHS: K(P_src, P_tgt)

Adaptive Multi-Head Manager:
----------------------------
- If `--jmmd-layers layer1` is specified:
    --> Instantiates 1 Projection Head network: ProjHead_Layer1 (176 -> 128 -> 64)
- If `--jmmd-layers layer1 layer2` is specified:
...
- If `--jmmd-layers layer1 layer2 layer3` is specified:
...
Usage Examples:
---------------
    # Multi-layer Projection Head JMMD with MATLAB A100 vs OpenNTN at SNR = 5 dB
    python run_JMMDpHead_LS_Attention.py --source-dir A100_2p18e9_600km_70deg_30kHz --target-dir DUR100nsFix_2p18G_600km_70deg_r15km_30to40mps --snr 5 --jmmd-layers layer1 layer2 --domain-weight 0.5 --save-features

    # Single-layer Projection Head JMMD on Transformer Encoder (Layer 1 only)
    python run_JMMDpHead_LS_Attention.py --snr 5 --jmmd-layers layer1 --domain-weight 0.5

    # Source-only baseline (no domain adaptation)
    python run_JMMDpHead_LS_Attention.py --snr 5 --only-source

    # Train with sample-wise zero-mean unit-variance standardization
    python run_JMMDpHead_LS_Attention.py --snr 5 --jmmd-layers layer1 layer2 --standardize

    # Quick test run (small subset, 5 epochs)
    python run_JMMDpHead_LS_Attention.py --test-code --jmmd-layers layer1 layer2
====================================================================================================
"""
...
ONLY_SOURCE = False             # Set True to train only on source (no JMMD)
JMMD_LAYERS = ["layer1", "layer2"]  # Default extracted layers for Projection Head JMMD
JMMD_MODE = "joint"            # "joint" (product kernel) or "layerwise" (additive sum)
PROJ_DIM = 64                  # Output dimension of each projection head
KERNEL_NUM = 5                 # Number of multi-scale Gaussian RBF kernels
KERNEL_MUL = 2.0               # Multiplier for Gaussian kernel bandwidths
...
# =============================================================================
# 3. NON-LINEAR PROJECTION HEAD (Adaptive RKHS Subspace)
# =============================================================================
class ProjectionHead(tf.keras.layers.Layer):
    """
    Lightweight Non-Linear Projection Head for JMMD Domain Alignment.
...
class HA02Model(tf.keras.Model):
    """
    HA02 Attention Model with configurable single/multi-layer feature extraction for JMMD.
    """
    ...
    def call(self, inputs, training=False, return_features=False, selected_layers=None):
        ...
        Args:
            ...
            selected_layers: List of layer names to extract, e.g. ['layer1', 'layer2']
        """
        ...
        if return_features:
            ...
            for lyr in selected_layers:
                ...
                if clean_lyr in feature_dict:
                    ...
                    raise ValueError(f"Unknown layer requested for JMMD: '{lyr}'. Available: {list(feature_dict.keys())}")
...
# =============================================================================
# 5. FAST VECTORIZED JMMD LOSS ON PROJECTED SUBSPACES
# =============================================================================
@tf.function
def compute_gaussian_kernel_matrix(x1: tf.Tensor, x2: tf.Tensor, kernel_mul: float = 2.0, kernel_num: int = 5, fix_sigma=None) -> tf.Tensor:
    """
    ...
    # L2 normalize feature vectors for numerical stability in RKHS
    ...
    dist_sq = tf.maximum(r1 - 2.0 * tf.matmul(x1_norm, x2_norm, transpose_b=True) + tf.transpose(r2), 0.0) # [B1, B2]
    
    if fix_sigma is not None:
        bandwidth = tf.cast(fix_sigma, tf.float32)
    else:
...
def compute_jmmd_loss(source_features: list, target_features: list,
    ...
    """
    Joint Maximum Mean Discrepancy (JMMD) Loss across projected representations.
    - 'joint': Computes joint product kernel K_joint = prod_{l=1}^L K_l
    - 'layerwise': Computes average MMD across layers
    """
...
def infer_full_dataset(model, H_perf: np.ndarray, H_in: np.ndarray, batch_size: int = 16, lower_range: int = -1, standardize: bool = False):
    ...
    for start in range(0, N, batch_size):
        ...
        x_sc, _, val1, val2 = preprocess_batch(y_batch, x_batch, lower_range, standardize=standardize)
        out_sc = model(x_sc, training=False)
        if standardize:
            out_real = deStandardize_ha02(out_sc, val1, val2).numpy()
        else:
...
def extract_features_with_heads(model, proj_heads: dict, H_in: np.ndarray, batch_size: int = 16,
                                selected_layers: list = None, lower_range: int = -1, standardize: bool = False):
    """
...
def plot_loss_curves(history: dict, save_dir: str):
    ...
    if 'train_jmmd_loss' in history:
        ax.plot(epochs, history['train_jmmd_loss'], label='Projected JMMD Loss', color='red', lw=1.5, ls='--')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.set_title('HA02 Projection-Head JMMD Loss Progression')
...
def main():
    parser = argparse.ArgumentParser(
        description="HA02 Attention Model with Multi-Layer Non-Linear Projection Head JMMD UDA for OpenNTN.",
    ...
    parser.add_argument('--only-source', action='store_true', default=ONLY_SOURCE, help="Train only on source data (no JMMD)")
    parser.add_argument(
        '--jmmd-layers', 
        nargs='+', 
        default=JMMD_LAYERS,
        help="Layers to extract for Projection Head JMMD alignment (e.g. '--jmmd-layers layer1 layer2' or '--jmmd-layers layer1')"
    )
    parser.add_argument('--jmmd-mode', type=str, default=JMMD_MODE, choices=['joint', 'layerwise'], help="JMMD kernel mode")
    parser.add_argument('--proj-dim', type=int, default=PROJ_DIM, help="Projection Head output dimension (default: 64)")
    parser.add_argument('--kernel-num', type=int, default=KERNEL_NUM, help="Number of Gaussian RBF kernels")
    parser.add_argument('--kernel-mul', type=float, default=KERNEL_MUL, help="Gaussian kernel multiplier")
    parser.add_argument('--domain-weight', type=float, default=0.5, help="JMMD loss weight (lambda)")
    ...
    print("=" * 80)
    print(f"HA02 Attention Model | Mode: {'Source-Only' if args.only_source else f'Projection Head JMMD ({args.jmmd_mode}) UDA'}")
    if not args.only_source:
        print(f"JMMD Extracted Layers: {selected_layers} ({len(selected_layers)} Dedicated Projection Head Networks)")
        print(f"Projection Head Output Dim: {args.proj_dim} (Projected Space: [{args.proj_dim}-D])")
    ...
    # Dynamic SSIM dynamic range
    ssim_max_val = 6.0 if args.standardize else (2.0 if args.lower_range == -1 else 1.0)

    # =========================================================================
    # COMPILED TRAINING STEP WITH PROJECTION HEAD JMMD
    # =========================================================================
    @tf.function
    def _train_step_jmmd_phead(x_src, y_src, x_tgt):
        with tf.GradientTape() as tape:
            ...
            # 4. Multi-Head Projected JMMD Loss
            proj_feats_src = []
            proj_feats_tgt = []
            for lyr, z_s, z_t in zip(selected_layers, raw_feats_src, raw_feats_tgt):
                phead = proj_heads[lyr]
                proj_feats_src.append(phead(z_s, training=True))
                proj_feats_tgt.append(phead(z_t, training=True))
            
            jmmd_loss = compute_jmmd_loss(
                proj_feats_src, proj_feats_tgt,
                kernel_mul=args.kernel_mul,
                kernel_num=args.kernel_num,
                mode=args.jmmd_mode
            ) if domain_weight > 0 else tf.constant(0.0, dtype=tf.float32)
            
            total_loss = est_loss + domain_weight * jmmd_loss
        ...
        return total_loss, est_loss, jmmd_loss
    ...
    print(f"\n[Train] Starting Projection-Head JMMD Training for {args.n_epochs} Epochs ...")
    ...
    for epoch in range(args.n_epochs):
        ...
        if args.save_features and epoch in feature_checkpoint_epochs:
            ...
            src_f = extract_features_with_heads(model, proj_heads, H_in_src[idx_train_src], args.batch_size, selected_layers, args.lower_range, standardize=args.standardize)
            tgt_f = extract_features_with_heads(model, proj_heads, H_in_tgt[idx_train_tgt], args.batch_size, selected_layers, args.lower_range, standardize=args.standardize)
        ...
        ep_loss, ep_est, ep_jmmd = 0.0, 0.0, 0.0
        ...
        for b in range(n_batches):
            ...
            if args.only_source:
                l_tot, l_est, l_jmd = _train_step_source_only(x_src_b, y_src_b)
            else:
                l_tot, l_est, l_jmd = _train_step_jmmd_phead(x_src_b, y_src_b, x_tgt_b)
        ...
        avg_jmmd = ep_jmmd / max(n_batches, 1)

        history['train_loss'].append(avg_loss)
        history['train_est_loss'].append(avg_est)
        history['train_jmmd_loss'].append(avg_jmmd)

        # Periodic Validation
        pred_val_src = infer_full_dataset(model, H_perf_src[idx_val_src], H_in_src[idx_val_src], args.batch_size, args.lower_range, standardize=args.standardize)
        val_nmse_src_db = compute_nmse_db(pred_val_src, H_perf_src[idx_val_src])
        history['val_nmse_src'].append(val_nmse_src_db)

        pred_val_tgt = infer_full_dataset(model, H_perf_tgt[idx_val_tgt], H_in_tgt[idx_val_tgt], args.batch_size, args.lower_range, standardize=args.standardize)
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0 or epoch == args.n_epochs - 1:
            print(f"Epoch {epoch+1:03d}/{args.n_epochs:03d} | Loss: {avg_loss:.4f} (Est: {avg_est:.4f}, Proj-JMMD: {avg_jmmd:.4f}) | Val Target NMSE: {val_nmse_tgt_db:.2f} dB")
    ...
    try:
        with open(txt_path, 'w') as f:
            ...
            f.write(f"Domain Adaptation:    {'Source-Only' if args.only_source else f'Projection Head JMMD ({args.jmmd_mode}) UDA'}\n")
            f.write(f"Normalization:        {norm_str}\n")
            f.write(f"JMMD Layers:          {selected_layers}\n")
    ...
    eval_dict = {
        ...
        'jmmd_layers': np.array(selected_layers)
    }
    ...
    print(f"\n[Done] Finished Projection-Head JMMD training and evaluation. Results saved in: {output_dir}")
