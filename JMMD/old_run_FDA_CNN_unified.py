...
DEFAULT_STRATEGY = 'fullTranslation1' # Choices listed below
...
# Import all train/val steps
import utils_GAN

# Map strategies to their training and validation function names
STRATEGY_MAP = {
    'fullTranslation1': {
        'train': 'train_step_cnn_residual_fda_fullTranslation1',
        'val': 'val_step_cnn_residual_fda'
    },
    'fullTranslation2': {
        'train': 'train_step_cnn_residual_fda_fullTranslation2',
        'val': 'val_step_cnn_residual_fda'
    },
    'rawThenScale': {
        'train': 'train_step_cnn_residual_fda_rawThenScale',
        'val': 'val_step_cnn_residual_fda'
    },
    'scaleCombined': {
        'train': 'train_step_cnn_residual_fda_scaleCombined',
        'val': 'val_step_cnn_residual_fda'
    },
    'jmmd_rawThenScale': {
        'train': 'train_step_cnn_residual_fda_jmmd_rawThenScale',
        'val': 'val_step_cnn_residual_fda'
    },
    'jmmd_rawThenScale_domainAware': {
        'train': 'train_step_cnn_residual_fda_jmmd_rawThenScale_domainAware',
        'val': 'val_step_cnn_residual_fda_jmmd_rawThenScale_domainAware'
    },
    'FDAfullTranslation1_jmmd': {
        'train': 'train_step_cnn_residual_FDAfullTranslation1_jmmd',
        'val': 'val_step_cnn_residual_fda'
    },
    'FDAfullTranslation1_coral_jmmd': {
        'train': 'train_step_cnn_residual_FDAfullTranslation1_coral_jmmd',
        'val': 'val_step_cnn_residual_FDAfullTranslation1_coral_jmmd'
    },
    'coral_jmmd': {
        'train': 'train_step_cnn_residual_coral_jmmd',
        'val': 'val_step_cnn_residual_coral_jmmd'
    },
    'jmmd_only': {
        'train': 'train_step_cnn_residual_jmmd',
        'val': 'val_step_cnn_residual_jmmd'
    },
    'source_only': {
        'train': 'train_step_cnn_residual_source_only',
        'val': 'val_step_cnn_residual_source_only'
    }
}

def main():
    parser = argparse.ArgumentParser(description="Unified FDA and JMMD CNN training runner.")
    ...
    parser.add_argument('--strategy', type=str, default=DEFAULT_STRATEGY, choices=list(STRATEGY_MAP.keys()), help="Translation/adaptation strategy to use")
    ...
    train_metrics = {
        ...
        'train_est_loss': [],
        'train_disc_loss': [],
        'train_domain_loss': [], # domain or JMMD/CORAL loss
    ...
    # Look up train and validation steps dynamically
    funcs = STRATEGY_MAP[args.strategy]
    train_step_func = getattr(utils_GAN, funcs['train'])
    val_step_func = getattr(utils_GAN, funcs['val'])
    ...
    for epoch in range(args.n_epochs):
        ...
        train_step_output = train_step_func(
        ...
        if (epoch == epoch_min) or (epoch + 1 > epoch_min and (epoch - epoch_min) % epoch_step == 0) and epoch != args.n_epochs - 1:
            H_sample, epoc_val_return = val_step_func(
            ...
            # Check if return_H_gen is accepted dynamically
            try:
                _, epoc_val_return, H_val_gen = val_step_func(
                    model, loader_H_eval, loss_fn, args.lower_range, weights=weights, linear_interp=linear_interp, return_H_gen=True
                )
            except TypeError:
                _, epoc_val_return = val_step_func(
                    model, loader_H_eval, loss_fn, args.lower_range, weights=weights, linear_interp=linear_interp
                )
                H_val_gen = np.zeros((1, 1)) # placeholder if not supported
        else:
            _, epoc_val_return = val_step_func(
                model, loader_H_eval, loss_fn, args.lower_range, weights=weights, linear_interp=linear_interp
            )
    ...
    print(f"Unified FDA/JMMD run completed successfully! Results stored in: {model_path}/{sub_folder}")
