...
Joint Maximum Mean Discrepancy (JMMD) Unsupervised Domain Adaptation (UDA) for 5G Non-Terrestrial 
Network (NTN) channel estimation.

JMMD Methodology:
-----------------
Joint Maximum Mean Discrepancy (JMMD) measures domain discrepancy across multiple latent feature 
representations by mapping feature distributions into a Reproducing Kernel Hilbert Space (RKHS) 
using multi-scale Gaussian RBF kernels:

    k(z_i, z_j) = sum_{m=1}^M exp( - ||z_i - z_j||^2 / (2 * sigma_m^2) )

For multi-layer joint domain adaptation across L extracted layers (e.g. Layer 1 + Layer 2):
    K_joint(x_i, x_j) = prod_{l=1}^L k_l(z_i^(l), z_j^(l))

The joint MMD objective minimizes the maximum mean discrepancy between the joint source and 
target distributions:
    L_JMMD = E[ K_joint(x_s, x_s') ] + E[ K_joint(x_t, x_t') ] - 2 * E[ K_joint(x_s, x_t) ]

HA02 Architecture & JMMD Layer Extraction:
------------------------------------------
...
Available JMMD Extraction Layers (`--jmmd-layers`):
- `layer1`: Transformer Encoder output (Z_enc) -> Shape: [B, 176] (Aligns pilot self-attention RKHS kernel)
...
---------------
    ...
    python run_JMMD_LS_Attention.py --snr 5 --only-source

    # Train with sample-wise zero-mean unit-variance standardization
    python run_JMMD_LS_Attention.py --snr 5 --jmmd-layers layer1 layer2 --standardize

    # Quick test run (small subset, 5 epochs)
    python run_JMMD_LS_Attention.py --test-code --jmmd-layers layer1 layer2
====================================================================================================
"""
...
JMMD_LAYERS = ["layer1", "layer2"]  # Default extracted layers for JMMD
...
# =============================================================================
# 4. FAST VECTORIZED JMMD / MMD LOSS COMPUTATION
# =============================================================================
@tf.function
def compute_gaussian_kernel_matrix(x1: tf.Tensor, x2: tf.Tensor, kernel_mul: float = 2.0, kernel_num: int = 5, fix_sigma=None) -> tf.Tensor:
    ...
    # L2 normalize feature vectors for robust RKHS geometry
    x1_norm = tf.nn.l2_normalize(x1_flat, axis=-1)
    x2_norm = tf.nn.l2_normalize(x2_flat, axis=-1)
    
    # Pairwise squared L2 distance: ||x1 - x2||^2
...
def compute_jmmd_loss(source_features: list, target_features: list,
    ...
    """
    Joint Maximum Mean Discrepancy (JMMD) Loss across multi-layer representations.
...
def extract_features(model, H_in: np.ndarray, batch_size: int = 16, selected_layers: list = None, lower_range: int = -1, standardize: bool = False):
...
def plot_loss_curves(history: dict, save_dir: str):
    ...
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.set_title('HA02 JMMD Training Loss Progression')
...
def main():
    parser = argparse.ArgumentParser(
        description="HA02 Attention Model with Multi-Layer Joint Maximum Mean Discrepancy (JMMD) UDA for OpenNTN.",
    ...
    parser.add_argument(
        ...
        nargs='+', 
        default=JMMD_LAYERS,
        help="Layers to extract for JMMD alignment (e.g. '--jmmd-layers layer1 layer2' or '--jmmd-layers layer1')"
    )
    ...
    print("=" * 80)
    print(f"HA02 Attention Model | Mode: {'Source-Only' if args.only_source else f'JMMD ({args.jmmd_mode}) UDA'}")
    if not args.only_source:
        print(f"JMMD Extracted Layers: {selected_layers} ({len(selected_layers)} Latent RKHS Spaces)")
    ...
    # =========================================================================
    @tf.function
    def _train_step_jmmd(x_src, y_src, x_tgt):
    ...
    print(f"\n[Train] Starting JMMD Domain Adaptation Training for {args.n_epochs} Epochs ...")
    ...
    for epoch in range(args.n_epochs):
        ...
        for b in range(n_batches):
            ...
            if args.only_source:
                ...
                l_tot, l_est, l_jmd = _train_step_jmmd(x_src_b, y_src_b, x_tgt_b)
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0 or epoch == args.n_epochs - 1:
            print(f"Epoch {epoch+1:03d}/{args.n_epochs:03d} | Loss: {avg_loss:.4f} (Est: {avg_est:.4f}, JMMD: {avg_jmmd:.4f}) | Val Target NMSE: {val_nmse_tgt_db:.2f} dB")
    ...
    try:
        with open(txt_path, 'w') as f:
            ...
            f.write(f"Domain Adaptation:    {'Source-Only' if args.only_source else f'JMMD ({args.jmmd_mode}) UDA'}\n")
    ...
    print(f"\n[Done] Finished JMMD training and evaluation. Results saved in: {output_dir}")
