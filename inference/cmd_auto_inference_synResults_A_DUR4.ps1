...
$labels = @(
    ...
    # "LI+DnCNN+CrossAttention Std Inferred",
    # "LS+Attention Inferred",
    # "LS+Attention Std Inferred",
    # "LS+DnCNN+Attention Inferred"
)
...
for ($i = 0; $i -lt $models.Length; $i++) {
    ...
    Write-Output "  Model Path   : $modelDir"
    Write-Output "  Inference Py : $inferScript"
    Write-Output "  Output Dir   : $outDir"
    Write-Output "------------------------------------------------------------"

    # Construct and invoke python batch inference script
    $pyArgs = @(
        $inferScript,
    ...
    Write-Output "Executing: python $($pyArgs -join ' ')"
    & python $pyArgs
...
# 7. Post-Inference Performance Evaluation using MATLAB script
Write-Output "======================================================================"
Write-Output " Starting Headless MATLAB Evaluation (syn_metrics_withBER)"
Write-Output "======================================================================`n"

$matlabExe = "matlab"
if (Test-Path "C:\Program Files\MATLAB\R2024b\bin\matlab.exe") {
