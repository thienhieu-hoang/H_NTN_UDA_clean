"""
Single-Dataset CNN Channel Estimator with Combined MSE & SSIM Loss (OpenNTN)
=============================================================================
...
This script uses a combined training loss function of MSE and SSIM.

Usage
-----
    python train_CNN_single_dataset_ssim.py --snr 10 --ssim-weight 0.1
    python train_CNN_single_dataset_ssim.py --snr -5  --input-type prac --epochs 300 --ssim-weight 0.2
"""
...
DEFAULT_SSIM_WEIGHT = 0.1          # weight of SSIM in combined loss (MSE: 1 - w, SSIM: w)
...
DEFAULT_SAVE_DIR    = ''           # auto = ./trained_models/SNR_{snr}dB_{input_type}_ssim
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Single-dataset CNN channel estimator (MSE + SSIM training loss).',
    ...
    parser.add_argument('--ssim-weight', type=float, default=DEFAULT_SSIM_WEIGHT,
                        help='Importance weight for SSIM loss (range [0.0, 1.0]). '
                             'Total Loss = (1 - w)*MSE + w*(1 - SSIM). '
                             'To balance scales, a value around 0.1 is recommended.')
    ...
    if not (0.0 <= args.ssim_weight <= 1.0):
        parser.error('--ssim-weight must be between 0.0 and 1.0 inclusive.')
    ...
    print('\n' + '=' * 58)
    print('  Single-Dataset CNN (MSE + SSIM Combination Loss)')
    print('=' * 58)
    ...
    print(f'  SSIM Weight   : {args.ssim_weight:.3f} (MSE Weight: {1.0 - args.ssim_weight:.3f})')
    ...
    if args.save_dir:
        ...
        save_dir = os.path.join(THIS_DIR, 'trained_models',
                                f'SNR_{args.snr}dB_{args.input_type}_ssim_w{args.ssim_weight}')
    os.makedirs(save_dir, exist_ok=True)
    ...
    for epoch in range(args.epochs):
        ...
        for b in range(n_train_batches):
            ...
            total_l, mse_l, ssim_l = _train_step(model, x_sc, y_sc, optimizer, loss_fn, lower_range, args.ssim_weight)
        ...
        for b in range(n_val_batches):
            ...
            comb_l, mse_l, ssim_l = compute_combined_loss(y_sc, x_corr, loss_fn, lower_range, args.ssim_weight)
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0 or epoch == args.epochs - 1:
            ...
            print(f'Epoch [{epoch+1:>4d}/{args.epochs}]  '
                  f'TrainLoss={avg_train_loss:.6f} (MSE={avg_train_mse:.6f}, SSIM={avg_train_ssim:.4f})  '
                  f'ValLoss={avg_val_loss:.6f} (MSE={avg_val_mse:.6f}, SSIM={avg_val_ssim:.4f})  '
            ...
            if args.save_model:
                ...
                with open(os.path.join(save_dir, 'best_epoch.txt'), 'w') as fh:
                    fh.write(f'best_epoch       = {best_epoch}\n'
                             ...
                             f'mse_weight       = {1.0 - args.ssim_weight}\n'
    ...
    print('\n' + '═' * 60)
    print('  EVALUATION RESULTS (Combined Loss)')
    print('═' * 60)
    print(hdr)
    print('─' * 60)
