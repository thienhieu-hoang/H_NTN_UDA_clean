...
class Pix2PixGeneratorLinear(Pix2PixGenerator):
    def __init__(self, output_channels=2, n_subc=132, gen_l2=None, 
                 dropOut_layers=['u1', 'u2'], dropOut_rate=0.3, extract_layers=['d2', 'd3', 'd4']):
        super().__init__(output_channels=output_channels, n_subc=n_subc, gen_l2=gen_l2,
                         dropOut_layers=dropOut_layers, dropOut_rate=dropOut_rate, extract_layers=extract_layers)
        kernel_regularizer = tf.keras.regularizers.l2(gen_l2) if gen_l2 is not None else None
        # Overwrite last layer to use linear activation (no activation)
...
def main():
    ...
    model          = GAN(n_subc=132, generator=Pix2PixGeneratorLinear, discriminator=PatchGANDiscriminator, gen_l2=None, disc_l2=1e-5)
