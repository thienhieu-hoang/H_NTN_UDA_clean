"""
Single-Dataset Attention-based Channel Estimator with Two-Stage Residual Error Refinement (OpenNTN)
===================================================================================================
Stage 1: Coarse reconstruction via Transformer-Encoder + Dense Upsampling (HA02).
Stage 2: Residual Error Network (Delta H) predicting high-frequency interpolation errors.
...
# =============================================================================
# 3. STAGE 2: MULTI-DILATION RESIDUAL ERROR REFINEMENT BLOCK
# =============================================================================
class ResidualRefineBlock(tf.keras.layers.Layer):
    """
    Second-stage CNN that models the systematic interpolation/extrapolation error:
        Delta_H = H_true - H_coarse
    Using multi-scale dilated convolutions to capture both subcarrier correlation (frequency)
    and symbol correlation (Doppler) simultaneously.
    """
    def __init__(self, filters=32, **kwargs):
        super(ResidualRefineBlock, self).__init__(**kwargs)
        self.filters = filters
        
        # Initial projection to feature space
        self.in_conv = tf.keras.layers.Conv2D(filters, (3, 3), padding='same', name="res_in_conv")
        
        # Residual Block 1: standard receptive field
        self.conv1_1 = tf.keras.layers.Conv2D(filters, (3, 3), padding='same', name="res_conv1_1")
        self.ln1 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="res_ln1")
        self.conv1_2 = tf.keras.layers.Conv2D(filters, (3, 3), padding='same', name="res_conv1_2")
        
        # Residual Block 2: Dilated in frequency (subcarriers)
        self.conv2_1 = tf.keras.layers.Conv2D(filters, (3, 3), dilation_rate=(2, 1), padding='same', name="res_conv2_1")
        self.ln2 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="res_ln2")
        self.conv2_2 = tf.keras.layers.Conv2D(filters, (3, 3), dilation_rate=(2, 1), padding='same', name="res_conv2_2")
        
        # Residual Block 3: Dilated in time (OFDM symbols)
        self.conv3_1 = tf.keras.layers.Conv2D(filters, (3, 3), dilation_rate=(1, 2), padding='same', name="res_conv3_1")
        self.ln3 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="res_ln3")
        self.conv3_2 = tf.keras.layers.Conv2D(filters, (3, 3), dilation_rate=(1, 2), padding='same', name="res_conv3_2")
        
        # Final projection to residual error (Real, Imag)
        self.out_conv = tf.keras.layers.Conv2D(2, (3, 3), padding='same', name="res_out_conv")

    def call(self, h_coarse, training=False):
        x = tf.nn.leaky_relu(self.in_conv(h_coarse))
        
        # ResBlock 1
        res1 = tf.nn.leaky_relu(self.ln1(self.conv1_1(x)))
        res1 = self.conv1_2(res1)
        x = x + res1
        
        # ResBlock 2 (Freq dilated)
        res2 = tf.nn.leaky_relu(self.ln2(self.conv2_1(x)))
        res2 = self.conv2_2(res2)
        x = x + res2
        
        # ResBlock 3 (Time dilated)
        res3 = tf.nn.leaky_relu(self.ln3(self.conv3_1(x)))
...
# =============================================================================
# 4. COMPLETE TWO-STAGE RESIDUAL REFINEMENT MODEL
# =============================================================================
class HA02ResidualRefineModel(tf.keras.Model):
    def __init__(self, num_pilot_elems=88, total_grid_elems=1848, num_channels=2, 
                 num_heads=2, n_filter=2, refine_filters=32, **kwargs):
        super(HA02ResidualRefineModel, self).__init__(**kwargs)
        ...
        self.refiner = ResidualRefineBlock(filters=refine_filters)
...
def save_channel_plots_pdf(H_perf_sample: np.ndarray,
    ...
    try:
        ...
        fig.colorbar(im, ax=ax)
        ax.set_title(f'HA02 ResidualRefine Output H_{input_type}_attention (Real Part) - {prefix.capitalize()} Sample 1', fontsize=14)
        ax.set_xlabel('Subcarrier Index' if H_perf_sample.shape[0] == 132 else 'Symbol Index', fontsize=12)
        ax.set_ylabel('Symbol Index' if H_perf_sample.shape[0] == 132 else 'Subcarrier Index', fontsize=12)
        plt.tight_layout()
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description='Train HA02 Attention with Two-Stage Residual Refiner.')
    ...
    parser.add_argument('--refine-filters', type=int, default=32, help='Filters in residual refiner')
    ...
    if args.save_dir:
        ...
        save_dir = os.path.join(THIS_DIR, 'trained_models_residual_refine', f'SNR_{args.snr}dB_{args.input_type}_{suffix}')
    os.makedirs(save_dir, exist_ok=True)
