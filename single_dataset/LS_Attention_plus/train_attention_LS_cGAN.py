"""
Single-Dataset Attention-based Channel Estimator with Conditional GAN (cGAN) Refinement (OpenNTN)
===================================================================================================
Stage 1: Coarse channel reconstruction via Transformer-Encoder + Dense Upsampling (HA02).
Stage 2: Conditional GAN (cGAN) Refiner:
         - Generator: Multi-scale residual refinement network predicting Delta_H (H_refined = H_coarse + Delta_H).
         - Conditional Discriminator (PatchGAN): Evaluates [H_coarse, H_candidate] pairs (4 channels)
           to penalize non-physical smoothing, enforce realistic multipath delay spreads, and recover
           fine-grained Doppler trajectories without blurring.
         - Inference: Pure feed-forward Generator execution (single pass, < 1.5 ms latency).
===================================================================================================
"""
...
DEFAULT_GEN_LR      = 1e-4
DEFAULT_DISC_LR     = 5e-5
DEFAULT_ADV_WEIGHT  = 0.005        # Adversarial loss weight
DEFAULT_FM_WEIGHT   = 0.001        # Feature matching loss weight
...
class TransformerEncoderBlock(tf.keras.layers.Layer):
    """
    Self-attention encoder operating across sparse pilot subcarriers.
    Maps [B, 88, 2] -> [B, 88, 2] with multi-head attention and GELU feed-forward.
    """
...
# =============================================================================
# 2. RESIDUAL CONVOLUTIONAL DECODER BLOCK (Dense Upsampler)
# =============================================================================
class ResidualConvDecoderBlock(tf.keras.layers.Layer):
    """
    Decodes attention-enhanced pilot features and upsamples them to full slot grid:
    [B, 88, 2] -> [B, 132, 14, 2].
    """
...
# =============================================================================
# 3. CGAN GENERATOR RESIDUAL REFINEMENT BLOCK
# =============================================================================
class cGANRefinerBlock(tf.keras.layers.Layer):
    """
    Multi-scale convolutional refiner acting as the refinement core of the Generator:
    Delta_H = G_refine(H_coarse)
    H_refined = H_coarse + Delta_H
    
    Employs multi-dilation residual blocks along frequency (subcarriers) and time (OFDM symbols)
    to capture both multipath delay spread profiles and Doppler phase dynamics.
    """
    def __init__(self, filters=32, **kwargs):
        super(cGANRefinerBlock, self).__init__(**kwargs)
        self.filters = filters
        
        # Initial projection
        self.in_conv = tf.keras.layers.Conv2D(filters, (3, 3), padding='same', name="cgan_in_conv")
        
        # Residual Block 1: Local smoothing
        self.conv1_1 = tf.keras.layers.Conv2D(filters, (3, 3), padding='same', name="cgan_conv1_1")
        self.ln1 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="cgan_ln1")
        self.conv1_2 = tf.keras.layers.Conv2D(filters, (3, 3), padding='same', name="cgan_conv1_2")
        
        # Residual Block 2: Frequency dilation (captures delay spread multipath)
        self.conv2_1 = tf.keras.layers.Conv2D(filters, (3, 3), dilation_rate=(2, 1), padding='same', name="cgan_conv2_1")
        self.ln2 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="cgan_ln2")
        self.conv2_2 = tf.keras.layers.Conv2D(filters, (3, 3), dilation_rate=(2, 1), padding='same', name="cgan_conv2_2")
        
        # Residual Block 3: Time dilation (captures Doppler phase evolution across slot)
        self.conv3_1 = tf.keras.layers.Conv2D(filters, (3, 3), dilation_rate=(1, 2), padding='same', name="cgan_conv3_1")
        self.ln3 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="cgan_ln3")
        self.conv3_2 = tf.keras.layers.Conv2D(filters, (3, 3), dilation_rate=(1, 2), padding='same', name="cgan_conv3_2")
        
        # Residual Block 4: Wider frequency dilation (captures long delay taps)
        self.conv4_1 = tf.keras.layers.Conv2D(filters, (3, 3), dilation_rate=(3, 1), padding='same', name="cgan_conv4_1")
        self.ln4 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="cgan_ln4")
        self.conv4_2 = tf.keras.layers.Conv2D(filters, (3, 3), dilation_rate=(3, 1), padding='same', name="cgan_conv4_2")
        
        # Final output projection
        self.out_conv = tf.keras.layers.Conv2D(2, (3, 3), padding='same', name="cgan_out_conv")

    def call(self, h_coarse, training=False, return_features=False):
        x = tf.nn.leaky_relu(self.in_conv(h_coarse), alpha=0.2)
        
        # ResBlock 1
        res1 = tf.nn.leaky_relu(self.ln1(self.conv1_1(x)), alpha=0.2)
        res1 = self.conv1_2(res1)
        x = x + res1
        
        # ResBlock 2 (Freq dilated)
        res2 = tf.nn.leaky_relu(self.ln2(self.conv2_1(x)), alpha=0.2)
        res2 = self.conv2_2(res2)
        x = x + res2
        
        # ResBlock 3 (Time dilated)
        res3 = tf.nn.leaky_relu(self.ln3(self.conv3_1(x)), alpha=0.2)
        res3 = self.conv3_2(res3)
        x = x + res3
        
        # ResBlock 4 (Wide freq dilated)
        res4 = tf.nn.leaky_relu(self.ln4(self.conv4_1(x)), alpha=0.2)
        res4 = self.conv4_2(res4)
        x = x + res4
        
        delta_h = self.out_conv(x)
        if return_features:
            return delta_h, x
        return delta_h

# =============================================================================
# 4. COMPLETE CGAN GENERATOR MODEL (Inference-Ready)
# =============================================================================
class HA02cGANGeneratorModel(tf.keras.Model):
    """
    Combined Generator Model:
    Stage 1: Sparse pilots H_LS [B, 88, 2] -> H_coarse [B, 132, 14, 2]
    Stage 2: H_coarse -> Delta_H [B, 132, 14, 2]
    Combined: H_refined = H_coarse + Delta_H
    """
    def __init__(self, num_pilot_elems=88, total_grid_elems=1848, num_channels=2,
                 num_heads=2, n_filter=2, refine_filters=32, **kwargs):
        super(HA02cGANGeneratorModel, self).__init__(**kwargs)
        ...
        self.refiner = cGANRefinerBlock(filters=refine_filters)
    ...
    def call(self, inputs, training=False, return_intermediate=False):
        ...
        if return_intermediate:
            delta_h, feat = self.refiner(h_coarse, training=training, return_features=True)
            h_refined = h_coarse + delta_h
            return h_refined, h_coarse, feat
        else:
            delta_h = self.refiner(h_coarse, training=training)
            h_refined = h_coarse + delta_h
            return h_refined

# =============================================================================
# 5. CONDITIONAL CHANNEL DISCRIMINATOR (PatchGAN)
# =============================================================================
class ConditionalChannelDiscriminator(tf.keras.Model):
    """
    Conditional PatchGAN Discriminator:
    Input: Concatenation of coarse condition [B, 132, 14, 2] and candidate channel [B, 132, 14, 2]
           -> Total input shape: [B, 132, 14, 4]
    Evaluates whether the candidate channel has physically realistic delay-Doppler fading statistics,
    coherent subcarrier phase trajectories, and sharp multipath peaks matching the pilot condition.
    
    Output: Patch validity map of shape [B, 17, 7, 1].
    """
    def __init__(self, filters=[32, 64, 128, 256], **kwargs):
        super(ConditionalChannelDiscriminator, self).__init__(**kwargs)
        self.conv1 = tf.keras.layers.Conv2D(filters[0], kernel_size=(4, 3), strides=(2, 1), padding='same', name="d_conv1")
        
        self.conv2 = tf.keras.layers.Conv2D(filters[1], kernel_size=(4, 3), strides=(2, 2), padding='same', name="d_conv2")
        self.ln2 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="d_ln2")
        
        self.conv3 = tf.keras.layers.Conv2D(filters[2], kernel_size=(3, 3), strides=(2, 1), padding='same', name="d_conv3")
        self.ln3 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="d_ln3")
        
        self.conv4 = tf.keras.layers.Conv2D(filters[3], kernel_size=(3, 3), strides=(1, 1), padding='same', name="d_conv4")
        self.ln4 = tf.keras.layers.LayerNormalization(epsilon=1e-5, name="d_ln4")
        
        # Patch-level validity prediction
        self.out_conv = tf.keras.layers.Conv2D(1, kernel_size=(3, 3), strides=(1, 1), padding='same', name="d_out")

    def call(self, h_coarse, h_candidate, training=False, return_features=False):
        # Concatenate condition and candidate: [B, 132, 14, 4]
        x = tf.concat([h_coarse, h_candidate], axis=-1)
        
        f1 = tf.nn.leaky_relu(self.conv1(x), alpha=0.2)
        f2 = tf.nn.leaky_relu(self.ln2(self.conv2(f1)), alpha=0.2)
        f3 = tf.nn.leaky_relu(self.ln3(self.conv3(f2)), alpha=0.2)
        f4 = tf.nn.leaky_relu(self.ln4(self.conv4(f3)), alpha=0.2)
        
        patch_out = self.out_conv(f4)
        
        if return_features:
            return patch_out, [f1, f2, f3, f4]
        return patch_out

# =============================================================================
# Helper Utilities & Data Handling
# =============================================================================
...
def load_mat_data(mat_path: str, input_type: str):
    ...
    if is_hdf5:
        ...
        with h5py.File(mat_path, 'r') as f:
            ...
            if input_key not in f or f[input_key].size == 0:
                for alt in [input_key, 'H_ls_pilots', 'H_ls_pilots_ori', 'H_li', 'H_li_ori', 'H_prac']:
                    if alt in f and f[alt].size > 0:
                        input_key = alt
                        break

            H_input_pilots = h5_to_complex(f[input_key][()])
            pilot_cols = np.array(f['pilot_cols']).squeeze().astype(int) - 1
            pilot_rows = np.array(f['pilot_rows']).squeeze().astype(int) - 1

            if H_input_pilots.ndim == 3:
                if H_input_pilots.shape[1] == 14 and H_input_pilots.shape[2] == 132:
                    H_input_pilots = np.transpose(H_input_pilots, (0, 2, 1))
                H_input_pilots = H_input_pilots[:, pilot_cols, pilot_rows]

            li_key = 'H_li' if 'H_li' in f else ('H_li_ori' if 'H_li_ori' in f else 'H_perfect')
            H_li_benchmark_grid = h5_to_complex(f[li_key][()])
            if H_li_benchmark_grid.ndim == 3 and H_li_benchmark_grid.shape[1] == 14 and H_li_benchmark_grid.shape[2] == 132:
                H_li_benchmark_grid = np.transpose(H_li_benchmark_grid, (0, 2, 1))
        ...
        H_perfect_ori = None
        for key in ['H_perfect_ori', 'H_perfect_original', 'H_true_ori', 'H_ori']:
            if key in mat and isinstance(mat[key], np.ndarray) and mat[key].size > 0:
...
def save_channel_plots_pdf(H_perf_sample: np.ndarray,
    ...
    try:
        ...
        fig.colorbar(im, ax=ax)
        ax.set_title(f'HA02 cGAN Output H_{input_type}_attention (Real Part) - {prefix.capitalize()} Sample 1', fontsize=14)
        ax.set_xlabel('Subcarrier Index' if H_perf_sample.shape[0] == 132 else 'Symbol Index', fontsize=12)
        ax.set_ylabel('Symbol Index' if H_perf_sample.shape[0] == 132 else 'Subcarrier Index', fontsize=12)
        plt.tight_layout()
...
def save_loss_plot_pdf(history: dict, save_dir: str):
    try:
        ...
        # GAN Loss Plot
        if 'train_g_adv' in history and 'train_d_loss' in history:
            fig, ax = plt.subplots(figsize=(8, 6))
            ax.plot(history['train_g_adv'], label='Gen Adv Loss', color='blue')
            ax.plot(history['train_d_loss'], label='Disc Loss', color='magenta')
            ax.set_xlabel('Epoch', fontsize=12)
            ax.set_ylabel('Loss', fontsize=12)
            ax.set_title('Training History - GAN Adversarial Loss', fontsize=14)
            ax.grid(True, linestyle='--', alpha=0.5)
            ax.legend()
            plt.tight_layout()
            plt.savefig(os.path.join(save_dir, 'loss_gan.pdf'), format='pdf')
            plt.close(fig)
...
# =============================================================================
# TRAINING STEP WITH CONDITIONAL GAN ADVERSARIAL LOSS
# =============================================================================
@tf.function
def _train_step_cgan(generator, discriminator, x_scaled, y_scaled, 
                     gen_optimizer, disc_optimizer, loss_fn, 
                     lower_range, ssim_weight, adv_weight, fm_weight,
                     use_huber=False, huber_delta=1.0, standardize=False,
                     gan_loss_type='lsgan'):
    """
    Simultaneous update of Generator and Conditional Discriminator:
    1. Discriminator update: evaluates [H_coarse, H_true] (real) vs [H_coarse, H_refined] (fake)
    2. Generator update: minimizes task loss (MSE/Huber + SSIM) + adversarial loss + feature matching
    """
    ...
    with tf.GradientTape() as disc_tape:
        h_refined, h_coarse, _ = generator(x_scaled, training=True, return_intermediate=True)
        
        # Real pair: [H_coarse, H_true]
        d_real, f_real = discriminator(h_coarse, y_scaled, training=True, return_features=True)
        # Fake pair: [H_coarse, H_refined] (detached from generator tape)
        d_fake, _ = discriminator(h_coarse, tf.stop_gradient(h_refined), training=True, return_features=True)
    ...
    with tf.GradientTape() as gen_tape:
        ...
        # Task Loss (MSE / Huber + SSIM)
        if use_huber:
            err = tf.abs(h_refined - y_scaled)
            huber_err = tf.where(
                err <= huber_delta,
                0.5 * tf.square(err),
                huber_delta * (err - 0.5 * huber_delta)
            )
            task_loss = tf.reduce_mean(huber_err)
            mse_loss = tf.reduce_mean(tf.square(h_refined - y_scaled))
            ssim_loss = tf.constant(0.0)
        else:
            mse_loss = loss_fn(y_scaled, h_refined)
            if standardize:
            ...
            ssim_val = tf_ssim(y_scaled, h_refined, max_val=max_val)
            ssim_loss = tf.reduce_mean(1.0 - ssim_val)
            task_loss = (1.0 - ssim_weight) * mse_loss + ssim_weight * ssim_loss
            
        # Adversarial Generator Loss
        d_fake_for_gen, f_fake = discriminator(h_coarse, h_refined, training=True, return_features=True)
        if gan_loss_type == 'lsgan':
            g_adv_loss = tf.reduce_mean(tf.square(d_fake_for_gen - 1.0))
        else:
            g_adv_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(d_fake_for_gen), logits=d_fake_for_gen))
            
        # Feature Matching Loss (regularizes discriminator feature space)
        if fm_weight > 0.0:
            fm_loss = 0.0
            for fr, ff in zip(f_real, f_fake):
                fm_loss += tf.reduce_mean(tf.abs(tf.stop_gradient(fr) - ff))
            fm_loss = fm_loss / tf.cast(len(f_real), tf.float32)
        else:
            fm_loss = tf.constant(0.0)
            
        g_total_loss = task_loss + adv_weight * g_adv_loss + fm_weight * fm_loss
        
    gen_grads = gen_tape.gradient(g_total_loss, generator.trainable_variables)
    gen_optimizer.apply_gradients(zip(gen_grads, generator.trainable_variables))
    
    return g_total_loss, task_loss, mse_loss, ssim_loss, g_adv_loss, d_loss
...
def export_model_to_onnx(model: tf.keras.Model, save_path: str, input_shape):
    ...
    try:
        ...
        print(f'[ONNX Export] Saved ONNX Generator model (architecture + weights) -> {save_path}')
    except Exception as e:
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description='Train HA02 Attention with Conditional GAN (cGAN) Refiner.')
    parser.add_argument('--snr', type=int, default=DEFAULT_SNR, help='Target SNR in dB')
    parser.add_argument('--epochs', type=int, default=DEFAULT_EPOCHS, help='Training epochs')
    parser.add_argument('--batch-size', type=int, default=DEFAULT_BATCH_SIZE, help='Batch size')
    parser.add_argument('--gen-lr', type=float, default=DEFAULT_GEN_LR, help='Generator learning rate')
    parser.add_argument('--disc-lr', type=float, default=DEFAULT_DISC_LR, help='Discriminator learning rate')
    parser.add_argument('--adv-weight', type=float, default=DEFAULT_ADV_WEIGHT, help='GAN adversarial loss weight')
    parser.add_argument('--fm-weight', type=float, default=DEFAULT_FM_WEIGHT, help='Feature matching loss weight')
    parser.add_argument('--gan-loss-type', type=str, default='lsgan', choices=['lsgan', 'bce'], help='GAN loss formulation')
    ...
    parser.add_argument('--save-model', action='store_true', help='Save best generator to ONNX')
    ...
    parser.add_argument('--refine-filters', type=int, default=32, help='Base filters in cGAN refiner')
    ...
    # Instantiate Generator and Conditional Discriminator
    generator = HA02cGANGeneratorModel(
        num_pilot_elems=H_input_pilots.shape[1], 
        total_grid_elems=14*132,
        refine_filters=args.refine_filters
    )
    discriminator = ConditionalChannelDiscriminator(
        filters=[args.refine_filters, args.refine_filters*2, args.refine_filters*4, args.refine_filters*8]
    )
    ...
    if args.save_dir:
        ...
        save_dir = os.path.join(THIS_DIR, 'trained_models_cgan_refine', f'SNR_{args.snr}dB_{args.input_type}_{suffix}')
    os.makedirs(save_dir, exist_ok=True)
    ...
    best_epoch = 0
    history = {
        'train_loss': [], 'train_mse': [], 'train_ssim': [], 'train_g_adv': [], 'train_d_loss': [],
        'val_loss': [], 'val_mse': [], 'val_ssim': []
    }

    print(f'[Train cGAN] {args.epochs} epochs | {n_train_batches} batches/epoch | Adv Weight: {args.adv_weight} | Loss: {args.gan_loss_type}')
    ...
    fm_weight_tf = tf.constant(args.fm_weight, dtype=tf.float32)
    ...
    for epoch in range(args.epochs):
        ...
        for b in range(n_train_batches):
            ...
            g_tot_l, task_l, mse_l, ssim_l, g_adv_l, d_l = _train_step_cgan(
                generator, discriminator, x_sc, y_sc,
                gen_optimizer, disc_optimizer, loss_fn,
                lower_range, ssim_weight_tf, adv_weight_tf, fm_weight_tf,
                use_huber=use_huber, huber_delta=huber_delta, standardize=standardize_tf,
                gan_loss_type=args.gan_loss_type
            )
            ep_train_loss += g_tot_l.numpy()
            ep_train_mse  += mse_l.numpy()
            ep_train_ssim += ssim_l.numpy()
            ep_train_g_adv += g_adv_l.numpy()
            ep_train_d_loss += d_l.numpy()
        ...
        for b in range(n_val_batches):
            ...
            y_pred_sc = generator(x_sc, training=False)
            if use_huber:
        ...
        history['train_g_adv'].append(ep_train_g_adv / max(n_train_batches, 1))
        history['train_d_loss'].append(ep_train_d_loss / max(n_train_batches, 1))
        ...
        if cur_val_loss < best_val_loss:
            ...
            best_epoch    = epoch + 1
            if args.save_model:
                export_model_to_onnx(generator, os.path.join(save_dir, 'best_model.onnx'), (1, H_input_pilots.shape[1], 2))

        if (epoch + 1) % 10 == 0 or epoch == 0 or (epoch + 1) == args.epochs:
            print(f'Epoch {epoch+1:03d}/{args.epochs:03d} | Train G: {history["train_loss"][-1]:.4e} (Adv: {history["train_g_adv"][-1]:.4e}) | D: {history["train_d_loss"][-1]:.4e} | Val: {cur_val_loss:.4e} (Best: {best_val_loss:.4e} @ ep {best_epoch})')

    if args.save_model:
        export_model_to_onnx(generator, os.path.join(save_dir, 'final_model.onnx'), (1, H_input_pilots.shape[1], 2))
    ...
    H_pred_train = infer_channel(generator, H_perfect[idx_train], H_input_pilots[idx_train], args.batch_size, lower_range, standardize=args.standardize)
    H_pred_val   = infer_channel(generator, H_perfect[idx_val], H_input_pilots[idx_val], args.batch_size, lower_range, standardize=args.standardize)
    H_pred_test  = infer_channel(generator, H_perfect[idx_test], H_input_pilots[idx_test], args.batch_size, lower_range, standardize=args.standardize)
    ...
    eval_dict = {
        ...
        'mmse_li_benchmark_train': mmse_li_benchmark_train, 'nmse_li_benchmark_train': mmse_li_benchmark_train, 'nmse_li_benchmark_train_db': nmse_li_benchmark_train_db, 'ssim_li_benchmark_train': ssim_li_benchmark_train,
